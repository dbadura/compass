...
func NewSecurityEventHandler(service SecurityEventService, logger *log.Logger) *SecurityEventsHandler {
	return &SecurityEventsHandler{service: service, logger: logger}
}

func (h *SecurityEventsHandler) Save(writer http.ResponseWriter, req *http.Request) {
...
func (h *SecurityEventsHandler) Get(writer http.ResponseWriter, req *http.Request) {
	id := mux.Vars(req)["id"]
	if len(id) == 0 {
...
func (h *SecurityEventsHandler) List(writer http.ResponseWriter, req *http.Request) {
...
func (h *SecurityEventsHandler) Delete(writer http.ResponseWriter, req *http.Request) {
	id := mux.Vars(req)["id"]
	if len(id) == 0 {
		httphelpers.WriteError(writer, errors.New("parameter id not provided"), http.StatusBadRequest)
	}
...
func (h *SecurityEventsHandler) closeBody(req *http.Request) {
	err := req.Body.Close()
	if err != nil {
